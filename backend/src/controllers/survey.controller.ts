...
export class SurveyController {
  ...
  static async createSurvey(req: Request, res: Response) {
    try {
      ...
      await query(
        ...
        [surveyId, title, description, JSON.stringify(questions), true, new Date(), new Date()]
      );
  ...
  static async submitResponse(req: Request, res: Response) {
    try {
      ...
      const metadata = await AgentKitService.processSurveyResponse(survey, responseObj);

      // Insert response
      await query(
        `INSERT INTO survey_responses (id, survey_id, responses, respondent_email, respondent_name, submitted_at, metadata)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [
          responseObj.id,
          survey_id,
  ...
  static async getResponses(req: Request, res: Response) {
    try {
      ...
      const result = await query(
        `SELECT id, responses, respondent_email, respondent_name, submitted_at, metadata
         FROM survey_responses
         WHERE survey_id = $1
         ORDER BY submitted_at DESC
         LIMIT $2 OFFSET $3`,
        [survey_id, limit, offset]
      );
   ...
   * Soft delete survey
   */
  static async softDeleteSurvey(req: Request, res: Response) {
    try {
      const { id } = req.params;

      // Check if survey exists
      const surveyResult = await query(
        'SELECT * FROM surveys WHERE id = $1 AND is_deleted = false',
        [id]
      );
  ...
  /**
   * Soft delete survey response
   */
  static async softDeleteResponse(req: Request, res: Response) {
    try {
      const { survey_id, response_id } = req.params;

      await query('UPDATE survey_responses SET is_deleted = true WHERE id = $1 AND survey_id = $2', [response_id, survey_id]);
